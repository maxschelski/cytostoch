...
def _execute_simulation_gpu(object_states, properties_array, times,
    ...
    while current_sim_nb < total_nb_simulations:
        ...
        while True:
            ...
            # if core_id == 0:
            #     print(timepoint_array[0, sim_id, param_id])
            ...
            if (timepoint_array[0, sim_id, param_id] >=
                ...
                if not new_simulation:
                    ...
                    #       nb_parallel_cores[sim_id, param_id])
                    # if core_id == 0:
                    #     print(111111111111, iteration_nb, sim_id)
                    break
    ...
    if thread_id < (nb_simulations * nb_parameter_combinations):
        ...
        while True:
            ...
            if assertion_checks:
                ...
                for state in range(1,int(np.max(object_states[0, 0,0]))+1):
                    state_mask = object_states[0, 0,0] == state
                    # if state == 1:
                    ...
                    nb_objects = len(object_states[0, 0,0][object_states[0, 0,0] == state])
                    ...
                    assert max_position < object_states[0, 0,0].shape[0]
...
def _run_iteration(object_states, properties_array , times,
    ...
    #     print(111, object_states[0, 0,1,sim_id, param_id])
    ...
    if reaction_times[sim_id, param_id] < 0:
        print(222333344, reaction_times[sim_id, param_id])
    ...
    if ((times[0, sim_id, param_id] +
        ...
        if core_id == 0:
            ...
            if reaction_times[sim_id, param_id] < 0:
                print(22233334455, reaction_times[sim_id, param_id])
        ...
        if object_states[0, 0, object_pos, sim_id, param_id] > 0:
            ...
            while creation_property < creation_on_objects.shape[2]:
                ...
                if (((math.isnan(creation_on_objects[-1, 0, 0])) |
                    ...
                    # if the orientation is opposite, property vals reduced
                    # the end value and therefore end was smaller than
                    # start, therefore end and start were switched so that start
...
def _get_total_and_single_rates_for_state_transitions(parameter_value_array,
    ...
    while transition_nb < last_transition_nb:
        ...
        if start_state == 0:
            ...
            # But if the generation of new objects depends on the number
            # of objects present, this density has to be calculated
            if not math.isnan(creation_on_objects[transition_nb, 0, 0]):
                density_sum = 0
        ...
        if object_states[0, 0, object_nb, sim_id, param_id] == start_state:
            ...
            while dependence_prop_nb < params_prop_dependence.shape[0]:
                if math.isnan(params_prop_dependence[dependence_prop_nb]):
                    ...
                    # are no more properties to consider
                    break

                if int(params_prop_dependence[dependence_prop_nb]) == 0:
    ...
    while object_nb < last_object_pos:
        if object_states[0, 0, object_nb, sim_id, param_id] == start_state:
            ...
            while dependence_prop_nb < params_prop_dependence.shape[0]:
                ...
                if int(params_prop_dependence[dependence_prop_nb]) == 0:
                    # position should always be added without modification
                    # by orientation
                    end_position += properties_array[0,
                                                     int(params_prop_dependence[
                                                             dependence_prop_nb]),
                ...
                # print(object_states[0, 0, object_nb, sim_id, param_id],
...
def _get_tmin_tmax_for_property_changes(property_changes_tminmax_array,
    ...
    while object_nb < last_object_pos:
        ...
        state = int(object_states[0, 0, object_nb, sim_id, param_id])
        if state > 0:
...
def _determine_positions_of_transitions(current_transitions,
    ...
    if start_state == 0:
        ...
        # position
        if core_id == 0:
            if object_states[0, 0,
                             first_last_idx_with_object[0,sim_id, param_id],
                             sim_id, param_id] != 0:
        ...
        if object_states[0, 0, first_last_idx_with_object[1, sim_id, param_id],
                         sim_id, param_id] == 0:
        ...
        if action_value != 0:
            ...
            (object_pos,
             last_object_pos) = _get_first_and_last_object_pos(
                nb_objects, nb_parallel_cores[sim_id, param_id], core_id)
            while object_pos < last_object_pos:
                object_state = int(object_states[0, 0, object_pos,
                                                 sim_id, param_id])
                if object_state == 0:
                    object_pos += 1
                    continue
                ...
                while action_property_nb < len(action_properties):
                    ...
                    # property_extreme_values array setup, at the 3rd dimension
                    # index 0 is threshold value or parameter value number
                    #       is it always the parameter value number?
                    # index 1 is closed (1) or not (0)
                    # index 2 is same dimension forward (1) or not
                    # index 3 and higher are the properties that should be added
                    ...
                    # this is checked by subtracting (for operation = 1) the
                        ...
                        if (object_states[0, 3, object_pos, sim_id, param_id] == 1):
                            while max_value_nb < len(max_value):
                                ...
                                if math.isnan(val_property_nb):
                                    break
                                # values are subtracted from max threshold
                                # since it should get more restrictive with
                                # other property values already extending the
                                # object towards the max threshold
                                if max_value[2] == 1:
                                    threshold = (threshold -
                                                 properties_array[0,
                                                                  int(val_property_nb),
                    ...
                    # for reverse oriented objects, the max value can only
                    # be reached by the property_nb 0, the position, since
                        ...
                        if min_value[1] == 0:
                            ...
                            if (object_states[0, 3, object_pos, sim_id, param_id] == -1):
                                while min_value_nb < len(min_value):
                                    ...
                                    # index 2 is always 1 thus far
                                    # values have to be added because the min
                                    # threshold becomes larger, therefore more
                                    # restrictive if the object already has
                    ...
                    #     print(111, object_states[0, 0, object_pos, sim_id, param_id],
                    #           new_property_val,
...
def _get_random_mt_at_position(position, object_states, properties_array,
    ...
    target_state = 3
    while object_nb < last_object_pos:
        if object_states[0, 0, object_nb, sim_id, param_id] == target_state:
            # check whether the starting point of the object is before the
            # target position
            object_pos = properties_array[0, object_nb, sim_id, param_id]
            # if object_states[0,3, obj]
            if object_pos < position:
                property_nb = 1
                while property_nb < properties_array.shape[0]:
                    object_pos += properties_array[property_nb, object_nb,
                                                 sim_id, param_id]
                if object_pos >= position:
        ...
        if (core_id == 0) & (math.isnan(creation_on_objects[transition_nb,
            ...
            if start_state != 0:
                ...
                if not math.isnan(transition_parameters[int(transition_nb), 1]):
                    ...
                    # object generation
                    object_states[0, 1, transition_position,
            ...
            if end_state != 0:
                ...
                # second dimension
                if object_states[0, 1, transition_position,
                                 sim_id, param_id] != 0:
                    ...
                    # transition_parameters.
                    if not math.isnan(
                            transition_parameters[int(object_states[0,
                                                          1, transition_position,
                                                          sim_id, param_id] - 1),
                                                  1]):
                        nb_objects_all_states[
                            1, int(transition_parameters[
                                       int(object_states[0, 1,
                                                         transition_position,
                                                         sim_id, param_id]) - 1,
                                       1]),
                                              sim_id, param_id] -= 1
                ...
                if object_states[0,2, transition_position,
                                 sim_id, param_id] != 0:
                    object_states[0,2, transition_position,
                                  sim_id, param_id] = 0
        ...
        # change property values based on transitions
            ...
            while property_nb < last_property_nb:
                ...
                if ((not math.isnan(creation_on_objects[
                    ...
                    # print(21, times[0, sim_id, param_id],
                    #       timepoint_array[0, sim_id, param_id])
                    # if the current transition allows inheritance of
                    # creation source (index 2 of object_states) or orientation
                    # (index 3 of object_states), inherit orientation and/or
                    # creation source

                    # Position 3 in transition_parameters
                    # orientation_dict = {"plus-end-out": 1,
                    #                     "plus-end-in": 2,
                    #                     "random": 3,
                    #                     "inherit": 4}
                    if ((transition_parameters[transition_nb, 2] == 2) |
                            (transition_parameters[transition_nb, 3] == 4)):
                        ...
                        # inherit creation source if required
                        if ((transition_parameters[transition_nb, 2] == 2) &
                                (object_states[0,2, template_object_position,
                                               sim_id, param_id] != 0)):
                            object_states[0,2, transition_position,
                ...
                #     print(222, object_states[0, 0, int(transition_position), sim_id, param_id],
                #           property_val,
                ...
                # print(23, times[0, sim_id, param_id],
                #       timepoint_array[0, sim_id, param_id])
...
def _cut_object(transition_position, transition_nb,
    ...
    nb_objects_all_states[0, int(object_states[0, 0, int(object_position),
                                               sim_id, param_id]),
                          sim_id, param_id] -= 1
    ...
    if state_before_cut == 0:
        ...
        while property_nb_tmp >= 0:
            ...
            properties_array[0, property_nb_tmp,
                           ...
                           param_id] = math.nan
            property_nb_tmp -= 1
        # object_states[0, 0, transition_position, sim_id, param_id] = 0
    ...
    if state_after_cut == 0:
        ...
        nb_objects_all_states[0,
                              int(object_states[0, 0, object_position,
                                                sim_id, param_id]),
                              sim_id, param_id] -= 1

        # remove time of object creation
        if (object_states.shape[0] > 1):
            object_states[1, 0, object_position, sim_id, param_id] = 0

        object_states[0, 0, object_position, sim_id, param_id] = 0
    else:
    ...
    # if length_tmp > 20:
    #     print(300001,
    #           object_states[0, 0, object_position, sim_id, param_id],
    #           object_states[0, 0, transition_position,sim_id, param_id]
    #           )
    # check if the idx for creating a new object is higher than
    # the currently highest idx
...
def _get_random_object_at_position(target_property_nbs, x_pos,
    ...
    while object_position <= first_last_idx_with_object[1, sim_id, param_id]+20:
        if object_states[0, 0, object_position, sim_id, param_id] > 0:
            ...
            # the position (property nb == 0) should not be multiplied by
            # the orientation
            start = properties_array[0, 0, object_position,
                                     sim_id, param_id]
            property_nb = 1
...
def _increase_lowest_no_object_idx(first_last_idx_with_object,
    ...
    while object_pos < object_states.shape[2]:
        if object_states[0, 0, object_pos, sim_id, param_id] == 0:
            break
    ...
    while removal_nb < len(all_object_removal_properties):
        ...
        while object_pos < last_object_pos:
            ...
            if math.isnan(combined_property_vals):
                ...
                # Object removal is for removing objects
                # that are completely out of the region.
                # For forward oriented objects this means that the position
                # plus length have to be out of the region.
                ...
                if remove_object:
                    ...
                    object_state_to_remove = int(object_states[0, 0,
                                                           int(object_pos),
                                                           sim_id, param_id])
                    ...
                    if object_states[0,1, int(object_pos),
                                     sim_id, param_id] != 0:
                        ...
                        object_states[0,1, int(object_pos),
                                      sim_id, param_id] = 0

                    if object_states[0,2, int(object_pos),
                                     sim_id, param_id] != 0:
                    ...
                    # remove time of object creation
                    if (object_states.shape[0] > 1):
                        object_states[1, 0, int(object_pos),
                                      sim_id, param_id] = 0

                    object_states[0, 0, int(object_pos),
                                  sim_id, param_id] = 0
